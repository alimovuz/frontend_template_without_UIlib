...
const CustomFileInput: FC<CustomFileInputProps> = ({ label, touched, error, name, className, placeholder, value, accept, labelClassName, onFileChange, isClear = false, type="file" }) => {
    ...
    return (
        <div className="grid gap-2">
            <label className={cn("flex justify-between", labelClassName)} htmlFor={name} ><span>{label}</span></label>
            ...
            <label htmlFor={name}
                ...
                {(file || value) && isClear && (
                    <img src={xIcon} alt="clear" onClick={handleClear} className={cn("w-5 h-5 cursor-pointer mr-2 opacity-0 group-hover:opacity-100 transition-opacity", "hover:text-red-500")} />
                )}
